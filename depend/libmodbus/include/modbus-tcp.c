...
static int _modbus_tcp_set_ipv4_options1(int s)
{
    int rc;
    int option;
...
static int _modbus_tcp_connect(modbus_t *ctx)
{
	...
	ctx->s = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP); //(PF_INET, SOCK_STREAM, 0);
    if (ctx->s == -1) {
        return -1;
    }
	...
	for(;;)
	{
		int ret = select(ctx->s+1, &fds, NULL, NULL, &tmOut);
		if (ret == 0)break;
		recv(ctx->s, m, 1, 0);
	}
    return 0;
